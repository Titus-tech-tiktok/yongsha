...
test('套图自动识别结果会复用缓存，仅新增或变化图片需要再次识别', async () => {
  ...
  const image = await sharp({ create: { width: 24, height: 24, channels: 3, background: '#eee8dc' } }).png().toBuffer();
  await Promise.all([
  ...
  await runtime.saveTemplateConfiguration({
    ...
    items: [
      ...
      { relativePath: '02-说明页.png', action: 'copy_original', reason: '说明页直接复制' }
    ]
  });
  ...
  assert.deepEqual(prepared.counts, { replacePrint: 1, copyOriginal: 1, exclude: 0, manualCheck: 0, copyTemplate: 1, skipCopy: 0 });
  ...
  await runtime.saveTemplateConfiguration({
    ...
    items: [
      ...
      { relativePath: '未选择.png', action: 'copy_original', reason: '本次不选' }
    ]
  });
  ...
  const generated = await runtime.generateTask({
    ...
    printPath,
    printName: '印花.png',
    masterImagePath: path.join(copyFolder, '无需换印花.png'),
  ...
  assert.deepEqual(
    await fs.readFile(path.join(generated.folder, '未选择.png')),
    await fs.readFile(path.join(copyFolder, '未选择.png'))
  );
