...
test('AI 缺失动作仍需人工确认，但旧版本和旧矩形区域可执行换印花', () => {
  const surface = {
    id: 'front',
    label: '正面白色柜门',
    polygon: [[0.2, 0.4], [0.8, 0.4], [0.8, 0.8], [0.2, 0.8]],
  ...
  assert.equal(oldVersion.action, 'replace_print');
  assert.equal(oldVersion.needs_manual_check, false);
  ...
  assert.equal(legacyRectangle.printableSurfaces.length, 1);
});
