...
3. 有清晰可见、边界明确的家具外侧白色/浅色柜门、抽屉正面或门板外表面时，processingMode=replace_print，并为每块面板输出精确多边形。
4. 图片仍有上架价值但没有可印花表面时，processingMode=copy_original，后端会逐字节复制原图，不调用生图 API。
5. 不要因为图片是多宫格、尺寸图、场景图或有人物遮挡就直接人工确认；只要能看清任意需要换印花的白色/浅色柜面，就优先 processingMode=replace_print，并只标注可执行面板。
6. AI 不允许选择 exclude；只有运营可以手动排除图片。

必须判定为 replace_print 的常见情况：
- 规格图、SKU 图、多宫格对比图里，每个小图只要有白色/浅色柜门，就逐个小图标注可印花柜门。
- 详情页场景图、卖点文字图、人物使用图里，只要有可见白色/浅色柜门或抽屉正面，就标注这些面板；文字、人物和道具写入 preserveAreas。
- 柜门打开或半开时，标注仍然可见的门板外表面和关闭柜门正面；柜体内部、门内侧、人物遮挡处不标注。
- 正面柜子被沙发、人物、植物、台面物品局部遮挡时，仍标注未被遮挡的可见白色/浅色柜面。
...
- 多宫格逐个小图确认；能识别的柜面输出 printableSurfaces，不能换印花的小图或遮挡区域写入 preserveAreas。
- confidence 只表示判断把握，不作为自动降级人工确认的硬门槛；有可执行面板时仍应输出 replace_print。
- replace_print 必须给出具体 printableArea 和至少一个有效 printableSurfaces 多边形；只有图像损坏、主体完全不可判断或可印花区域无法形成任何有效面板时，才使用 manual_check。`;
...
第一张输入图是套图文件夹中选出的母版参考图，只用于确定产品结构、门数、正面比例、柜脚、边框、门缝、把手和基础视角。
第二张输入图是原始印花图，必须作为柜门/抽屉/面板表面图案的唯一来源。

生成要求：
- 输出一张干净、完整、可作为后续整套图迁移标准的电商产品母版图。
- 保持第一张参考图里的家具品类、门数、柜体结构、比例、边框、柜脚、把手和门缝关系。
...
- 不要保留参考图里的尺寸线、标注线、无关标签、水印或说明文字；如果参考图是尺寸图，只保留产品本体。
